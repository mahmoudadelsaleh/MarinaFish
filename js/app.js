...
function refreshAllSlots(){
  document.querySelectorAll("[data-slot]").forEach(slot=>{
    ...
    // تم إزالة const item = findItem(name); لأنها لم تكن مستخدمة هنا
    if(line){
...
function totals(){
  let sub=0,qty=0; 
  cart.forEach(l => { sub += l.unitPrice*l.qty; qty += l.qty });
...
function renderCartLines(){
  ...
  // الحل الجذري لمشكلة السلة: استخدام forEach بدلاً من map
  let html = "";
  cart.forEach(l => {
    html += `
    <div class="line">
