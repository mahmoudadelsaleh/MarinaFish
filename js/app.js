...
// التأكد من جلب البيانات بشكل صحيح من ملف التكوين (config)
const sectionsData = window.__SECTIONS__ || [];

function renderNav() {
    const navList = document.getElementById("nav-list");
    if (!navList) return;
    navList.innerHTML = sectionsData.map(s =>
...
function addItem(encName) {
    ...
    if (cur) {
        ...
        cart.set(name, { name, unitPrice: parsePrice(it.price), priceLabel: it.price, qty: 1 });
    }
    refreshAllSlots();
    updateCartUI();
}
...
function renderCartLines() {
    ...
    // تم التعديل هنا: استخدام forEach لبناء النص بدلاً من map لضمان توافقية جميع المتصفحات
    let htmlContent = "";
    cart.forEach(l => {
        htmlContent += `
        <div class="line">
...
// تم التعديل هنا: التأكد من تحميل كافة عناصر الـ HTML قبل تنفيذ دوال الرسم
